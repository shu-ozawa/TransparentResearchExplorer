...
Managed by `backend/api/endpoints/papers.py`. This group provides functionalities for searching papers on arXiv and for processing/scoring individual papers.

-   **`POST /search` (New Endpoint)**
    -   **Purpose**: Search for papers on arXiv based on a query string. This endpoint is used by the frontend to fetch papers when a user selects a query from the query tree or enters a query manually. It directly queries arXiv without interacting with the local cache, returning a list of paper metadata.
    -   **Request Body**: `SearchRequest` schema (`{ "query": "string" }`)
    -   **Outputs**: `List[PaperInfo]` schema. Each `PaperInfo` object contains:
        -   `title` (str)
        -   `authors` (List[str])
        -   `abstract` (str)
    -   **ArXiv Interaction**:
        -   Uses an `ArxivClient` (dependency injected, likely from `backend.api.arxiv_client.py`) to perform the search.
        -   Calls the `search` method of the `ArxivClient` with the provided query and a hardcoded `max_results` of 20.
        -   The results from arXiv are formatted into a list of `PaperInfo` objects.
    -   **Note**: This endpoint provides raw search results from arXiv and does not seem to interact with the database caching mechanism present in `/api/queries/search`.
...
-   **`POST /search`**
    ...
    -   **Cache Interaction**:
        1.  For each query in the input list, the system first uses the `ArxivAPIClient` (via `get_arxiv_client`) to get a list of paper `entry_id`s from arXiv.
...
-   **Interaction**:
    -   The `ArxivAPIClient` class encapsulates the logic for interacting with arXiv. (Note: `backend/api/endpoints/papers.py` uses a direct dependency `ArxivClient`. If this is different from `ArxivAPIClient`, this section may need further refinement to describe both clients if both are in use, or to reflect the correct primary client.)
    -   It uses the official `arxiv` Python library.
    -   The `search_papers` method of `ArxivAPIClient` takes a keyword and maximum number of results, queries the arXiv API, and transforms the results into a list of `ArxivPaper` Pydantic schema objects. The `search` method of `ArxivClient` (used in `/api/papers/search`) takes a query and max_results.
    -   **Retry Logic**: The `ArxivAPIClient` incorporates retry functionality using the `tenacity` library. If an API call to arXiv fails (e.g., due to `ArxivHTTPError` or `ArxivUnexpectedEmptyPageError`), it will automatically retry the request a few times with exponential backoff, enhancing the robustness of the integration.
...
-   **Location**: Defined in `backend/schemas/arxiv_schema.py` (for arXiv-specific structures) and also inline within the respective endpoint files in `backend/api/endpoints/` for other request/response bodies (e.g., `SearchRequest`, `PaperInfo` in `papers.py`).
